...
function calcularPontos(valorCentavos: number) {
...
export const clienteBuscarFidelidade = createServerFn({
  method: "GET",
}).handler(async () => {
  ...
  const [agendamentosConcluidos, pagamentosAssinatura] =
    await Promise.all([
  ...
  const eventosAtendimentos = agendamentosConcluidos.map(
    (agendamento) => {
      const valorCentavos =
  ...
  const eventos = [...eventosAtendimentos, ...eventosAssinaturas].sort(
    (a, b) =>
      new Date(b.data).getTime() - new Date(a.data).getTime(),
  );

  const pontosTotais = eventos.reduce(
    (total, evento) => total + evento.pontos,
    0,
  );

  const proximaRecompensa =
    recompensas.find(
      (recompensa) => recompensa.pontosNecessarios > pontosTotais,
    ) ?? null;

  const pontosParaProximaRecompensa = proximaRecompensa
    ? proximaRecompensa.pontosNecessarios - pontosTotais
    : 0;

  const metaAtual =
    proximaRecompensa?.pontosNecessarios ??
    recompensas[recompensas.length - 1].pontosNecessarios;

  const progresso = Math.min(
    100,
    Math.round((pontosTotais / metaAtual) * 100),
  );
